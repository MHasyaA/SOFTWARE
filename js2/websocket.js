...
const WSManager = {
  ...
  _dispatch(data) {
    switch(data.topic) {
      case 'dashboard/update':
        ...
        break;
      case 'trend/data':
        STATE.trendData = data.load;
        STATE.emit('trendData', data.load);
        break;
      default:
  ...
  send(topic, load) {
    ...
    if (this._useMock) {
      ...
      this._handleMockUplink(topic, load);
      return true;
    }
  ...
  _scheduleReconnect() {
    ...
    this.reconnectCount++;
    const delay = Math.min(CONFIG.WS_RECONNECT_INTERVAL * this.reconnectCount, 30000);
  ...
  _startMock() {
    ...
    this.mockTimer = setInterval(() => this._pushMockData(), 3000);
  },
  ...
  _mockState: {
    units: [
      ...
      { id: 'COMP-04', running: true,  alarm: true,  alarms: ['HIGH_TEMP', 'HIGH_CURRENT'] },
  ...
  _pushMockData() {
    ...
    const rnd = (base, variance) => (base + (Math.random() - 0.5) * variance * 2).toFixed(1);
    ...
    const payload = {
      ...
      presets: {
        schedules: [
          { id: 1, start: '08:00', end: '17:00', assigned_on: ['COMP-01', 'COMP-02'], is_matching: true  },
          { id: 2, start: '17:00', end: '22:00', assigned_on: ['COMP-03'],            is_matching: false },
          { id: 3, start: '22:00', end: '06:00', assigned_on: ['COMP-05', 'COMP-06'], is_matching: false },
          { id: 4, start: '06:00', end: '08:00', assigned_on: ['COMP-07'],            is_matching: false }
        ],
        pressure: [
          { id: 1, pressure_high: '7', pressure_low: '5', assigned_high: ['COMP-01', 'COMP-02'], assigned_low: ['COMP-01'] },
          { id: 2, pressure_high: '8', pressure_low: '6', assigned_high: ['COMP-04', 'COMP-05'], assigned_low: ['COMP-04'] },
          { id: 3, pressure_high: '7', pressure_low: '5', assigned_high: ['COMP-06', 'COMP-07'], assigned_low: ['COMP-06'] }
        ]
      },
  ...
  _sendMockTrend(req) {
    const points = req.range === '1d' ? 24 : req.range === '7d' ? 7 * 24 : 30 * 24;
    const now = Date.now();
    const intervalMs = { '1d': 3600000, '7d': 3600000, '30d': 3600000 }[req.range];

    const baseValues = { pressure: 6.2, voltage: 387, current: 29, power: 15.5 };
    const base = baseValues[req.metric] || 6;

    const data = Array.from({ length: points }, (_, i) => ({
      timestamp: new Date(now - (points - i) * intervalMs).toISOString(),
      value:     parseFloat((base + Math.sin(i * 0.3) * base * 0.1 + (Math.random() - 0.5) * base * 0.05).toFixed(2))
    }));

    setTimeout(() => {
      this._dispatch({ topic: 'trend/data', load: { unit: req.unit, metric: req.metric, range: req.range, data } });
    }, 400);
  }
};
